...
namespace WebApplication1.Controllers.User
{
    ...
    public class UserProductsController : ControllerBase
    {
        ...
        public UserProductsController(MobileDbContext context)
        {
            _context = context;
        }
        ...
        [HttpGet("search2")]
        public async Task<IActionResult> SearchByName(string? name = "", string? system = "", int? categoryId = null)
        {
            var query = _context.products.AsQueryable();
            if (!string.IsNullOrEmpty(system))
            {
                query = query.Where(x => x.System.ToLower().Contains(system.ToLower()));
            }
